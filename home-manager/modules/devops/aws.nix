...
in
{
  ...
  sops.secrets."aws/secret_access_key" = {
    ...
    ${pkgs.awscli2}/bin/aws configure --profile ${aws_profile_name} set aws_access_key_id $(cat ${config.sops.secrets."aws/access_key".path})
    ${pkgs.awscli2}/bin/aws configure --profile ${aws_profile_name} set aws_secret_access_key $(cat ${config.sops.secrets."aws/secret_access_key".path})
    '';
